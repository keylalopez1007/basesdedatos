...
function clearSpecialLoops() { if (specialTimerId) clearInterval(specialTimerId); if (specialSpawnId) clearTimeout(specialSpawnId); specialTimerId = null; specialSpawnId = null; const target = document.querySelector('#special-target'); if (target) { target.hidden = true; target.onclick = null; target.onpointerdown = null; } }
...
async function endGeneralGame(messageText) {
  ...
  try { const result = await request('/partida/finalizar', { method: 'POST' }); document.querySelector('#special-room').hidden = true; document.querySelector('.scene-wrap').hidden = false; showGameOverThenReport(result.reporte, messageText); document.querySelector('#start-game-button').textContent = 'Nueva partida'; showMessage(messageText); } catch (error) { showMessage(error.message); }
}
...
function handleSpecialClick(type) { if (!specialRoomActive) return; if (type === 'heart') { specialHp = Math.min(100, specialHp + 15); specialFeedback(`♥ Pulso recuperado (+15): ${Math.round(specialHp)}%`, 'success'); } else { specialHp = Math.max(0, specialHp - 18); specialFeedback('☠ Error crítico', 'danger'); document.querySelector('#special-room').classList.add('damage-flash'); setTimeout(() => document.querySelector('#special-room').classList.remove('damage-flash'), 180); } updateSpecialHud(); if (specialHp >= 100) finishSpecial(true); if (specialHp <= 0) finishSpecial(false); }
function spawnSpecialTarget() { if (!specialRoomActive) return; const target = document.querySelector('#special-target'); const type = Math.random() < .7 ? 'heart' : 'skull'; target.textContent = type === 'heart' ? '♥' : '💀'; target.className = `special-target ${type}`; target.dataset.specialType = type; target.dataset.clicked = 'false'; target.style.left = `${12 + Math.random() * 76}%`; target.style.top = `${15 + Math.random() * 68}%`; target.hidden = false; target.onpointerdown = (event) => { event.preventDefault(); event.stopPropagation(); if (target.hidden || target.dataset.clicked === 'true') return; target.dataset.clicked = 'true'; target.hidden = true; handleSpecialClick(type); }; setTimeout(() => { if (specialRoomActive) target.hidden = true; }, 1000); specialSpawnId = setTimeout(spawnSpecialTarget, 1100); }
document.querySelector('#special-arena').addEventListener('click', (event) => { const target = event.target.closest('#special-target'); if (!target || target.hidden || !specialRoomActive || target.dataset.clicked === 'true') return; target.dataset.clicked = 'true'; target.hidden = true; handleSpecialClick(target.dataset.specialType || (target.classList.contains('heart') ? 'heart' : 'skull')); });
...
function showGameOverThenReport(report, text) { const gameOver = document.querySelector('#game-over-screen'); const finalReport = document.querySelector('#final-report'); hud.hidden = true; startPanel.hidden = false; finalReport.hidden = true; gameOver.hidden = false; document.querySelector('#game-over-message').textContent = text; setTimeout(() => { gameOver.hidden = true; showReport(report); }, 2600); }
