...
function spawnSpecialTarget() { if (!specialRoomActive) return; const target = document.querySelector('#special-target'); const type = Math.random() < .7 ? 'heart' : 'skull'; target.textContent = type === 'heart' ? '♥' : '💀'; target.className = `special-target ${type}`; target.dataset.specialType = type; target.style.left = `${12 + Math.random() * 76}%`; target.style.top = `${15 + Math.random() * 68}%`; target.hidden = false; setTimeout(() => { if (specialRoomActive) target.hidden = true; }, 550); specialSpawnId = setTimeout(spawnSpecialTarget, 900); }
document.querySelector('#special-arena').addEventListener('click', (event) => { const target = event.target.closest('#special-target'); if (!target || target.hidden || !specialRoomActive) return; target.hidden = true; handleSpecialClick(target.dataset.specialType || (target.classList.contains('heart') ? 'heart' : 'skull')); });
